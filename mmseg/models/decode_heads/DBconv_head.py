...
class Attention(nn.Module):
    def __init__(self,
        ...
        if use_rpb:
            ...
            if global_mode:
                rpb_list.append(
                    nn.Parameter(torch.empty(num_heads, image_size[0] * image_size[0], image_size[1] * image_size[1]),
                                 requires_grad=True))
        ...
        if not self.global_mode:
            ...
            if hasattr(self, 'rpb'):
                if attn.size(-1) != self.rpb[-1].size(1) or x.size(-2) != self.rpb[-1].size(2):
                    attn = attn + F.interpolate(self.rpb[-1].unsqueeze(0), size=attn.shape[2:], mode='bicubic',
                                                align_corners=False)
                else:
